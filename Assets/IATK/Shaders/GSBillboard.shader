...
{    Properties
{
	...
	_spriteHalfSize("Sprite half size",Float) = 0.025
	_gaussianHeight("Gaussian function's a",Float) = 1
	_gaussianOffset("Gaussian function's b",Float) = 0.5
	_hScaleFactor("h's scaling factor",Float) = 1
	_alphaCoeff("Particles' alpha",Range(0,1)) = 0.001
...
SubShader
	{
		...
		struct app_data
		{
			...
			float4 color : COLOR;
			float3 normal : NORMAL; // store h - length scale or v - velocity
		...
		float _gaussianHeight;
		float _gaussianOffset;
		float _hScaleFactor;
		float _spriteHalfSize;
		float _alphaCoeff;
		...
		float getGaussian(float x, float y, float a, float b, float c) {
			// f(x) = a . exp(- ((x-b)^2 + (y-b)^2)) / 2.c^2)
			return a * exp(-((x - b) * (x - b) + (y - b) * (y - b)) / (2 * c * c));
		}
		...
		void geom(point v2g input[1], inout TriangleStream<g2f> triStream)
		{
			...
			// Access instanced variables
			float Size = UNITY_ACCESS_INSTANCED_PROP(Props, _Size) * input[0].color.w * 4 * _hScaleFactor;
			...
			float halfS = _spriteHalfSize * (Size + (dist * sizeFactor));
		...
		f_output frag(g2f input)
		{
			...
			output.color.w = tex2D(_MainTex, input.tex0.xy).w * _alphaCoeff;

			output.depth = output.color.a > 0.5 ? input.pos.z : 0;


			/// OBSOLETE
			//float h = input.color.w;

			// Testing Gaussian alpha
			//float x = input.tex0.x;
			//float y = input.tex0.y;                  
			//output.color.a *= getGaussian(x,y, _gaussianHeight, _gaussianOffset, h * _hScaleFactor);
